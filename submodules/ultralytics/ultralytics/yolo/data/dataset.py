...
class YOLODataset(BaseDataset):
    ...
    def __init__(self,
                 ...
                 names=None,
                 classes=None,
                 lvc_chain: torchvision.transforms.Compose | None = None,
                 do_print: bool = True,
                 ):
        self.use_segments = use_segments
        self.use_keypoints = use_keypoints
        self.names = names
        self.lvc_chain = lvc_chain
    ...
    def cache_labels(self, path=Path('./labels.cache')):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            pbar = tqdm(results, desc=desc, total=total, bar_format=TQDM_BAR_FORMAT, disable=(not self.do_print))
    ...
    def get_labels(self):
        ...
        if exists and LOCAL_RANK in (-1, 0):
            ...
            tqdm(None, desc=self.prefix + d, total=n, initial=n, bar_format=TQDM_BAR_FORMAT, disable=(not self.do_print))  # display cache results
            if cache['msgs']:
    ...
    def build_transforms(self, hyp=None):
        if self.augment:
            ...
            transforms = Compose([LetterBox(new_shape=(self.imgsz, self.imgsz), scaleup=False, lvc_chain=self.lvc_chain)])
        transforms.append(
