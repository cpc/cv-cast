...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if self.lvc_chain is not None:
            img = self.lvc_chain(img)

        if shape[::-1] != new_unpad:  # resize
            img = img.reshape(-1, new_unpad[1], new_unpad[0])
        img = torch.nn.ConstantPad2d((left, right, top, bottom), 114 / 255)(img)
        img = img * 255
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            if nl:
                ...
                masks = torch.zeros(1 if self.mask_overlap else nl, h // self.mask_ratio,
                                    w // self.mask_ratio)
            labels['masks'] = masks
        if self.normalize:
