...
from ultralytics.yolo.utils import (DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, RANK, ROOT, callbacks,
                                    is_git_dir, yaml_load, suppress_stdout_stderr)
...
class YOLO:
    ...
    def __init__(self, model: Union[str, Path] = 'yolov8n.pt', task=None, session=None,
    ...
    def get_val_dataloader(self, data=None,
            lvc_chain: transforms.Compose | None = None,
            num_batches: int | None = None, do_print: bool = True, **kwargs):
        """Get a dataloader used for validation
        """
        ...
        args.lvc_chain = lvc_chain
        args.num_batches = num_batches
        args.do_print = do_print

        validator = TASK_MAP[self.task][2](args=args)
        validator.init_data(model=self.model, lvc_chain=lvc_chain)
        return validator.dataloader


    @smart_inference_mode()
    def val(self, data=None, lvc_chain: transforms.Compose | None = None,
            num_batches: int | None = None, do_print: bool = True, **kwargs):
        """
        ...
        validator = TASK_MAP[self.task][2](args=args)
        stats = validator(model=self.model, lvc_chain=lvc_chain)
        validator.metrics.json_res = {}
        validator.metrics.json_res["metrics/mAP50-95(B)"] = stats["metrics/mAP50-95(B)"]
        self.metrics = validator.metrics
        self.num_batches = validator.args.num_batches
    ...
    def probe(self, probe_config: dict, lvc_chain: transforms.Compose | None = None, do_print: bool = True, **kwargs):
        """
        Trains the model on a given dataset.
        ...
        if overrides.get('resume'):
            overrides['resume'] = self.ckpt_path
        overrides["probe_config"] = probe_config
        overrides["lvc_chain"] = lvc_chain
        overrides["do_print"] = do_print

        self.task = overrides.get('task') or self.task

        if self.task != "detect":
            raise ValueError(f"Probing task {self.task} is not supported yet.")
        ...
        res = self.trainer._do_probe()
        self.num_batches = self.trainer.args.num_batches
        return res
